...
class Account(ABC):
    def __init__(self, account_no, balance):
        self.balance = ""  # Initialize balance as an empty string (should be removed or assigned to balance parameter)
        self.account_no = account_no
        self._balance = balance  # Use the provided balance parameter
        self.transactions = []  # Initialize an empty list to store transactions

    def deposit(self, amount):
        self.transactions.append(('deposited Amount', amount))  # Add deposit transaction to the list
        self._balance += amount  # Update the balance by adding the deposited amount
        self.save_transactions()  # Save the transactions to a file
    ...
    def balance_enquiry(self):
        return self.balance()  # Call the balance method (should be self._balance)

    def save_transactions(self):
        folder_name = "transaction_history"
        os.makedirs(folder_name, exist_ok=True)  # Create the transaction history folder if it doesn't exist

        filename = os.path.join(folder_name, f"{self.account_no}.txt")  # Filename based on the account number
...
class Customer:
    ...
    def load_account_data(self):
        ...
        os.makedirs(folder_name, exist_ok=True)  # Create the customer data folder if it doesn't exist
